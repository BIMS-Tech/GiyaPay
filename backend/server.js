...
// Helper function to generate a unique filename for new articles
const generateUniqueFilename = () => {
    const existingArticles = fs.readdirSync(BLOG_DIR).filter(file => file.startsWith('article-') && file.endsWith('.html'));
    let maxNum = 0;
    existingArticles.forEach(file => {
        const num = parseInt(file.replace('article-', '').replace('.html', ''));
        if (!isNaN(num) && num > maxNum) {
            maxNum = num;
        }
    });
    return `article-${maxNum + 1}.html`;
};
